...
export default class Container {
  ...
  glob(pattern, options = {}) {
    ...
    filenames.forEach((filename) => {
      ...
      const result = attempt(() => {
        this._register({ filename, namespace, options: opts });
      });
      if (result instanceof Error) {
        debug(`error registering component: ${filename}`);
      }
      return null;
    });
   ...
   * @private
   */
  _register({ filename, namespace, options }) {
    const mod = require(`${options.dir}/${filename}`); // eslint-disable-line
    let main = mod;
  ...
  _registerNamespaced({ filename, main, mod, namespace }) {
    if (!(typeof mod[namespace] === 'object' && !Array.isArray(mod[namespace]))) {
      throw new Error(`Invalid namespace declaration found for component: ${filename}`);
    }
    const options = mod[namespace];
    defaults(options, this.options.defaults || {});
    const component = new Component({ filename, mod: main, options });
